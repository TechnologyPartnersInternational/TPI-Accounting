...
import { ChevronLeft, ChevronRight, Edit, Eye, Download } from 'lucide-react';
...
export function SearchResultsTable({ jobs, exportButtonLabel = 'Export to Excel' }: SearchResultsTableProps) {
  ...
  const handleExport = async () => {
    try {
      ...
      if (response.ok) {
          ...
          a.href = url;
          a.download = `Filtered_Job_Export_${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col">
      ...
      <div className="px-6 py-4 flex justify-between items-center border-b border-gray-200 bg-gray-50/50">
          <h2 className="text-sm font-semibold text-gray-800">Results Table ({jobs.length})</h2>
          <button 
           onClick={handleExport}
           disabled={jobs.length === 0}
           className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white rounded-lg text-sm font-medium transition-colors shadow-sm"
          >
        ...
        <table className="w-full text-sm text-left border-collapse min-w-[1200px]">
          ...
          <tbody className="divide-y divide-gray-200 text-gray-700 bg-white">
            {groupedJobs.length === 0 ? (
              <tr>
                <td colSpan={12} className="px-6 py-12 text-center text-gray-500 font-medium bg-white">
                  No jobs found matching your advanced search criteria.
                </td>
              </tr>
            ) : (
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-6 py-4 border-t border-gray-200 bg-gray-50/50">
          <span className="text-sm text-gray-500">
            Showing <span className="font-semibold text-[#0f172a]">{(currentPage - 1) * ITEMS_PER_PAGE + 1}</span> to <span className="font-semibold text-[#0f172a]">{Math.min(currentPage * ITEMS_PER_PAGE, jobs.length)}</span> of <span className="font-semibold text-[#0f172a]">{jobs.length}</span> results
          </span>
          <div className="flex gap-1 border border-gray-200 rounded-lg overflow-hidden bg-white shadow-sm">
            ...
            <button className="px-4 py-1.5 text-sm font-semibold bg-indigo-50 text-indigo-600 border-r border-gray-200">{currentPage}</button>
            <button 
